...
Az indulaskori RPM spike-ot a SimpleFOC 700 ms-os zero elektromos szog alingazasi impulzusa okozza,
amit `kHallZeroElectricAngle = 0.0f` beallitassal ki lehet hagytani (ha `kUseFixedHallSensorDirection = true`).

Az atadasnal a PID-et nem nullazzuk ujra, hogy ne essen vissza hirtelen a
feszultsegparancs.
Az atadasi PID allapot a startup feszultseggel elotoltott, es a Hall watchdog
az interruptban latott Hall elek alapjan is frissul.
Startup assist es closed-loop alatt ugyanaz az egy celrampa fut. Az assist open-loopban
koveti a rampat, majd a handoff utan a closed-loop ugyanonnan folytatja. A rampa
0 RPM-rol indul; az indulasi erot a startup assist magasabb feszultseglimitje adja,
nem egy sebessegugras.
Az atadas feltetele nem csak egy fix RPM: a mert Hall RPM-nek az aktualis
celrampa megadott reszet is kovetnie kell.
